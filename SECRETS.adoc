...
|DOCKERHUB_USERNAME | UserName to log on to DockerHub | Repository | github action workflows (check)
|DOCKERHUB_TOKEN | Token to log on to DockerHub | Repository | github action workflows (check)
|SONAR_TOKEN | Token to log on to SounarCloud | Repository | github action workflows (check)
|===
...
| systemProp.sonar.organization | jococo-ch | Organisation configured in Sonar
