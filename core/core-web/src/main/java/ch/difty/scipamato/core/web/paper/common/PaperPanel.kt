...
abstract class PaperPanel<T>(
    ...
    private fun queueHeaderFields() {
        ...
        queue(newNavigationButton("previous", FontAwesome7IconType.backward_step_s, { pm.hasPrevious() }) {
            pm.previous()
            pm.itemWithFocus
        })
        queue(newNavigationButton("next", FontAwesome7IconType.forward_step_s, { pm.hasNext() }) {
            pm.next()
            pm.itemWithFocus
        })
        ...
        val addRemoveNewsletter: BootstrapAjaxLink<Unit> = object :
            ...
            private val iconType: IconType
                get() = if (!isAssociatedWithNewsletter)
                    FontAwesome7IconType.square_plus_s
                else if (isAssociatedWithWipNewsletter)
                    FontAwesome7IconType.envelope_open_r
                else FontAwesome7IconType.envelope_r
        }
